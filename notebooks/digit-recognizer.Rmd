...
```{r, include = FALSE}
training <- data <- read.csv("~/projects/kaggle/digit-recognizer/data/raw/train.csv")
...
```{r}
train.data <- training[,2:785]/255
labels <- training[,1]
test.data <- testing/255
...
```{r}
train.labels <- to_categorical(labels)
```
