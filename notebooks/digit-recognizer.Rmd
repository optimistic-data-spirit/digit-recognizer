...
Interesting to note that the format in which the data is provided on the Kaggle plateform is already reshaped from the initial 28 x 28 pixel images. So no need to do this step.

The data provided on the Kaggle website is also different than the original MNIST dataset in the sense that only 42000 of the original 60000 training set where provided. To test the accuracy of the mmodel to be build, one can set aside 10% of the training dataset to test before submitting to the website for evaluation.
...
So good practice is to take the dataset and split it two datasets (80% for the training of the model and 20% for the testing of the model). However, confusingly, the datasets provided by Kaggle are named train.csv and test.csv.

```{r, include = FALSE}
mnist_data <- read.csv("~/projects/kaggle/digit-recognizer/data/raw/train.csv")
predictions <- read.csv("~/projects/kaggle/digit-recognizer/data/raw/test.csv")
...
```{r}
mnist_data_images <- mnist_data[,2:785]/255
mnist_data_labels <- mnist_data[,1]
prediction_images <- testing/255

#Change dataset to matrix
mnist_data_images <- data.matrix(mnist_data_images)
prediction_images <- data.matrix(prediction_images)
```

RENDUE ICI - NOW HAVE TO SPLIT THE DATA INTO TRAINING AND TESTING BEFORE PREDICTIONS.
...
```{r train}
history <- network %>%
  fit(train_data, train_labels, epochs = 25, batch_size = 86)
```
